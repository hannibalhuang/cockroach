...
//
// The sample duration is an interval length in nanoseconds; datapoints which
// fall into the same interval will be aggregated together into a single Sample.
//
// The key duration is also a length in nanoseconds, which is used to determine
// how many intervals are grouped into a single InternalTimeSeriesData
// message. This value must be a multiple of the sample duration.
//
...
func (ts TimeSeriesData) ToInternal(keyDuration int64, sampleDuration int64) (
	...
	if keyDuration%sampleDuration != 0 {
		return nil, util.Errorf(
			"error converting TimeSeriesData to InternalTimeSeriesData: "+
	...
	for _, dp := range ts.Datapoints {
		...
		if dp.IntValue == nil && dp.FloatValue == nil {
			return nil, util.Errorf(
				"error converting TimeSeriesData to InternalTimeSeriesData: "+
					"datapoint %v has no value.",
				dp)
		}
		if dp.IntValue != nil && dp.FloatValue != nil {
			return nil, util.Errorf(
				"error converting TimeSeriesData to InternalTimeSeriesData: " +
					"datapoint %v has both integer and float data.",
			)
		}
